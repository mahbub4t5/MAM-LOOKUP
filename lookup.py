...
# CONFIG
# =========================================================
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8673660611:AAH4GeKUXEdRaKerXHV8DuevLvO34EwiGpU")
...
def count_referral_if_valid(user_id):
    ...
    execute(
        "UPDATE users SET ref_points = ref_points + 3, current_limit = current_limit + 3 WHERE chat_id = ?",
        (referrer_id,)
    )
...
def user_reply_kb():
    ...
    if KeyboardButtonRequestUsers:
        try:
            ...
            kb.row(
                KeyboardButton("👤 Select User", request_users=request, style="primary"),
                KeyboardButton("🔍 Lookup by ID", style="success")
            )
        except TypeError:
            kb.row(
                KeyboardButton("👤 Select User", style="primary"),
                KeyboardButton("🔍 Lookup by ID", style="success")
            )
    else:
        kb.row(
    ...
    kb.row(
        ...
        KeyboardButton("🔵 Redeem", style="success")
    )
    ...
    kb.row(
        KeyboardButton("🔴 Developer", style="primary"),
        KeyboardButton("🟣 Refer & Earn", style="success")
    )
...
def lookup_success_text(target_id, target_name, phone_masked, country, api_name, extra, remaining):
    text = (
        ...
        "║  📌  <b>Result</b>\n"
        "╚━━━━━━━━━━━━━━━━━━━━━━━━━╝\n"
        f"❱ <b>Phone</b> ❱ <code>{phone_masked or 'Not found'}</code>\n"
        f"❱ <b>Country</b> ❱ {country or '-'}\n"
    )
...
def help_text():
    return (
        ...
        "🟣 <b>Refer & Earn</b>\n"
        "❱ Get +3 limit for each successful referral.\n\n"
        "⚠️ <b>Note</b>\n"
...
def user_buttons(message):
    ...
    if message.text == "🟣 Refer & Earn":
        ...
        text = (
            ...
            "🎁 <b>Reward:</b> 3 limit per successful referral\n\n"
...
def general_text_handler(message):
    ...
    if state == "waiting_target_id":
        ...
        threading.Thread(
            target=process_lookup,
            args=(message, target_id, "Manual ID"),
            daemon=True
        ).start()
        return
