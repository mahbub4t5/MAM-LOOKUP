...
# CONFIG
# =========================================================
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8673660611:AAG2yTSnNy54s3WdMrtMwi9l6TDH-IOJ-sc")
...
def extract_lookup_result(data):
    ...
    if isinstance(data, dict):
        # নতুন API ফরম্যাট: { "result": { ... }, "success": true }
        if "result" in data and isinstance(data["result"], dict):
            res = data["result"]
            if res.get("success") == True:
                phone = res.get("number")
                country = res.get("country")
                country_code = res.get("country_code") or ""
                # developer বা key_expiry দেখাতে চাইলে extra তে যোগ করুন
                if data.get("developer"):
                    extra.append(f"Developer: {data['developer']}")
            else:
                # success false, যেমন "Phone number not found"
                extra.append(res.get("msg", "Not found"))
        else:
            # পুরনো ফরম্যাট (যেমন igfollows.site) ব্যাকওয়ার্ড কম্প্যাটিবিলিটি
            inner = data.get("data")
...
def user_buttons(message):
    ...
    if message.text == "🔍 Lookup by ID":
        ...
        bot.send_message(
            message.chat.id,
            "🔍 <b>Send Telegram User ID</b>\n\nExample: <code>8955959693</code> (ID)",
            reply_markup=user_reply_kb()
        )
        return
...
def general_text_handler(message):
    ...
    if state == "waiting_target_id":
        ...
        # ইউজারের নাম বের করার চেষ্টা (এখানে ইন্ডেন্টেশন ঠিক করে দিলাম)
        user_row = fetchone(
