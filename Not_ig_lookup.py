...
# CONFIG
# =========================================================
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8673660611:AAGzQsiqD_Dj8lV7kIt28BtJj0SgrhiiEHI")
...
def seed_default_api():
    ...
    """, (
        "V1",
        "https://telegramtonumtrial.vercel.app/?key=anuragxtrial&q={user_id}",
        now_str()
    ))
...
def extract_lookup_result(data):
    ...
    if isinstance(data, dict):
        ...
        if isinstance(phone_info, dict):
            ...
            number = phone_info.get("number") or ""

            if number:
                phone = f"{country_code}{number}" if country_code else number

            for k in ["tg_id", "message", "success"]:
                if phone_info.get(k) not in [None, ""]:
                    extra.append(f"{k}: {phone_info.get(k)}")
...
def user_reply_kb():
    ...
    if KeyboardButtonRequestUsers:
        try:
            ...
            kb.row(
                KeyboardButton("🔍 Lookup by ID or @", style="success"),
...
def help_text():
    return (
        ...
        f"║  📖 <b>{BOT_NAME} HELP</b> 📖  ║\n"
        "╚━━━━━━━━━━━━━━━━━━━━━━━━━╝\n\n"
        "🔍 <b>Lookup by ID or Username</b>\n"
        "❱ You can lookup using a Telegram numeric user ID or Username.\n\n"
        "👤 <b>Select User</b>\n"
...
def user_buttons(message):
    ...
    if message.text == "🔍 Lookup by ID or @":
        set_state(message.chat.id, state="waiting_target_id")
        bot.send_message(
            message.chat.id,
            "🔍 <b>Send Telegram User ID or Username</b>\n\nExample: <code>8955959693</code> (ID) or <code>@username</code>",
            reply_markup=user_reply_kb()
        )
        return
...
def general_text_handler(message):
    ...
    if state == "waiting_target_id":
        ...
        if not target_id:
            bot.send_message(
                message.chat.id,
                lookup_fail_text("Empty input. Send a valid Telegram ID or username."),
                reply_markup=user_reply_kb()
            )
            clear_state(message.chat.id)
            return

        target_query = target_id.lstrip("@")
        ...
        if target_query.isdigit():
            ...
            user_row = fetchone(
                "SELECT full_name, username FROM users WHERE lower(username) = lower(?)",
                (target_query,)
            )
        ...
        threading.Thread(
            target=process_lookup,
            args=(message, target_query, target_name),
            daemon=True
        ).start()
        return
